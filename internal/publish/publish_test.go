...
func newTestServer(t *testing.T) *httptest.Server {
	...
	mux.HandleFunc("/api/archives", func(res http.ResponseWriter, req *http.Request) {
		...
		_, err := res.Write([]byte(fmt.Sprintf(`{"data": {"signed-id": "123","filename":"slug.tar.gz"}, "meta":{"upload-url": "https://%s/upload-blob", "headers": {"Content-Type":"application/gzip"}}}`, req.Host)))
		if err != nil {
			t.Fatalf("Failed to write response: %s", err)
		}
	})
